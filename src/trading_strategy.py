...
def calculate_return(ticker, time_interval, pct_buy, pct_sell, year, month):
    ...
    for m in range(1, len(times_by_day)):
        ...
        for n in range(0, len(times_by_day[m])-2):
            ...
            if (not bought_to_open and (pct_change < -1*pct_buy) and not sold_to_open and curr_time.hour < 10 and curr_time.minute < 40):
                limit_hit = True
                limit_val = curr_val + curr_val * stop_limit_pct
                if (limit_hit and curr_val > limit_val):
            ...
            if (not sold_to_open and (pct_change > pct_buy) and not bought_to_open and curr_time.hour < 10 and curr_time.minute < 40):
